    ...
    internal class OneDriveHelper
    {
        ...
        internal async Task Sync(OneDriveInfo info)
        {
            ...
            using (_tracer.Step("Applying {0} changes ...", changes.Count))
            {
                ...
                ApplyResult deletionResult = await ApplyChangesParallel(changes.DeletionChanges, info.AccessToken);
                ApplyResult fileChangeResult = await ApplyChangesParallel(changes.FileChanges, info.AccessToken);
                // apply folder changes at last to maintain same timestamp as in OneDrive
                ApplyResult directoryChangeResult = await ApplyChangesParallel(changes.DirectoryChanges, info.AccessToken);
                ...
                if (totalFailCount > 0)
                {
                    ...
                    throw new Exception(string.Format(CultureInfo.CurrentUICulture, Resources.OneDriveDeploymentFailure, totalFailCount));
                }
            }
        ...
        private ChangesResult GetChanges(OneDriveModel.OneDriveItemCollection items, Dictionary<string, OneDriveModel.ItemInfo> ids, string rootUri)
        {
            ChangesResult result = new ChangesResult();
        ...
        private string GetPath(Dictionary<string, OneDriveModel.ItemInfo> ids, OneDriveModel.OneDriveItem item)
        {
            var path = item.name;
        ...
        private async Task<ApplyResult> ApplyChangesParallel(OneDriveModel.OneDriveChangeCollection changes, string accessToken)
        {
        ...
        {
            bool deleted = await RetryHandler(async () =>
            {
                await Task.Run(() =>
                {
